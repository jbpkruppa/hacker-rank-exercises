...
function testabbreviation(a, b) {
    let n, m;

    n = a.length;
    m = b.length;
    let d = new Array(n + 1).fill(new Array(m + 1));
    for (let j = 0; j <= m; j++) {
        if (j == 0)
            d[0][j] = 1;
        else d[0][j] = 0;
...
function abbreviation(a, b) { // passed 13/16. Stucked in case of a possibility of uppercasing a lowercase and avoiding next Uppercase do match... need to Use DynamicProgramming!!!!!
    let currB = 0;
    let lastRemovable = false; //see if it there another way by removing last intead of making it uppercase
    for (let char of a) {
        console.log(`charA: ${char}, currB: ${b[currB]}`);
        if (char == char.toUpperCase()) {
            if (char != b[currB]) {
                if (lastRemovable == false) return 'NO';
                else{
                    if (char== b[currB-1]){
                        //pass!
                        console.log('removing last uppercasing...')
                        lastRemovable = false;
                    }else{
                        return 'NO';
                    }
            ...
            } else if(char.toUpperCase() == b[currB]){
                currB++;
                lastRemovable = true;
            }
        }
